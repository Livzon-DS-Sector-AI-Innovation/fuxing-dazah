"""AI 配置中心 store — 运行时大脑（镜像 safety/ai_config/store.py）。

- **读（热路径，全同步）**：``get_profile`` / ``get_profile_config`` 只读内存缓存，
  零 IO 零锁；缓存 miss / TTL 过期时经同步 psycopg 只读会话重建（每 profile 一次查询）。
- **字段级回退**：起点 = registry.default_config 深拷贝 → env 层（env_map 非空且
  os.getenv 有值 → 覆盖，sources=env）→ DB 层（活行 enabled=true 且字段非空 →
  覆盖，sources=db）；DB 行 enabled=false → 整行回落 env/default，status="disabled"。
- **status 五态**：``db`` / ``env`` / ``default`` / ``disabled`` / ``missing``。
- **写（async，API 层传入 session）**：``set_profile`` 校验 → 软删行恢复 → 应用 →
  before/after 审计（api_key 一律脱敏）→ flush → select re-fetch → invalidate(profile)。
- **TTL 兜底**：缓存 60s 惰性过期，防御多实例/漏失效。
- **测试接缝**：构造函数 ``row_loader`` 注入预设行（返回模拟行或 None）；
  生产不传，走 ``_db.make_sync_session_factory`` 同步只读会话。DB 读失败仅告警并
  回落 env/registry 默认——配置中心故障不中断业务。
"""
...
from app.modules.warehouse.models import AiConfigAudit, AiModelProfile
...
# 测试接缝：row_loader(profile) -> 模拟行（含 profile/config/enabled/is_deleted 属性）或 None
ProfileRowLoader = Callable[[str], Any]
...
class ProfileView:
    ...
    api_key_masked: str                          # 展示脱敏
    enabled: bool                                # 活行 enabled（无 DB 行时当 True）
    status: ProfileStatus
    sources: dict[str, SourceKind]


class AiConfigStore:
    """模块级单例。热路径全同步（内存缓存），写操作 async（DB + 审计 + 失效）。"""

    def __init__(self, row_loader: ProfileRowLoader | None = None) -> None:
        self._profile_cache: dict[str, ProfileView] = {}
        self._loaded_at: dict[str, float] = {}
        self._ttl_seconds: float = _TTL_SECONDS
        self._row_loader = row_loader  # 测试注入；None = 生产同步会话
    ...
    def get_profile(self, profile: str) -> ProfileView:
        """某 profile 合并视图；未知 profile 抛 ValueError（404 语义）。"""
    ...
    async def warmup(self) -> None:
        """启动预热：全量加载 profile 行（失败仅告警，读路径回退 env/默认）。"""
    ...
    async def set_profile(
        ...
        - 未知 profile 抛 ValueError（404 语义）；越界值抛 ValueError（422 语义）；
        - api_key 空串/None = 不改（保留 DB 现值）；非空覆盖；
        - 软删行恢复；before/after 审计（api_key 一律脱敏）；
        - UPDATE 后 ``flush()`` → ``select`` re-fetch → ``invalidate(profile)``。
        """
    ...
    def _build_profile_view(self, profile: str, row: Any | None) -> ProfileView:
        ...
        for field in info.field_names:
            ...
            if not env_name:
                continue
            env_value = getattr(get_settings(), env_name, None)
        ...
        # status 归属：DB 行禁用优先（整行回落 env/default）；其次 db/env/default
    ...
    def _coerce_env_value(field: str, raw: str, info: registry.ProfileInfo) -> Any:
        """env 值类型对齐 default_config（int/float 字段）；非法值回落默认。"""
    ...
    # ═══════════════════════════════════════════════════════════
    # 内部：DB 访问（读路径同步会话；测试经构造函数 row_loader 注入）
    # ═══════════════════════════════════════════════════════════
    ...
    def _fetch_profile_row(self, profile: str) -> AiModelProfile | None:
        ...
        try:
            if self._row_loader is not None:
                return cast("AiModelProfile | None", self._row_loader(profile))
    ...
    def _validate_profile_payload(profile: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        规则：
        - 未知 profile 抛 ValueError（404 语义，get_profile 已预检）；
        - profile 无关字段（不在 field_names/enabled/note 内）→ ValueError（422）；
        - 0 <= temperature <= 2；1 <= timeout <= 600；256 <= max_tokens <= 65536；
        - api_key 空串/None → 不修改（不进返回）；非空最长 512；
        ...
        for key, raw in payload.items():
            ...
            if key == "api_key":
                ...
                if isinstance(raw, bool) or not isinstance(raw, int) or not (256 <= raw <= 65536):
                    raise ValueError("max_tokens 必须在 256-65536 之间")
                values[key] = raw
            elif key == "timeout":
                if isinstance(raw, bool) or not isinstance(raw, int) or not (1 <= raw <= 600):
                    raise ValueError("timeout 必须在 1-600 之间")
                values[key] = raw
            else:  # registry 断言保证 field_names ⊆ default_config
                values[key] = raw
        return values
