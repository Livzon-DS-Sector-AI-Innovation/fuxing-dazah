    ...
    steps=[
        ToolStep(
            ...
            inputs=[
                ...
                ToolInput(key="mode", label="核对方式", type="select", options=["按月核对", "按日期范围核对"], required=True),
                ToolInput(key="month", label="月份（YYYY-MM）", type="text", placeholder="如 2026-08"),
                ToolInput(key="start_date", label="开始日期（YYYY-MM-DD）", type="text", placeholder="如 2026-08-01"),
                ToolInput(key="end_date", label="结束日期（YYYY-MM-DD）", type="text", placeholder="如 2026-08-24"),
            ],
        ),
        ToolStep(
            id="dedupe",
