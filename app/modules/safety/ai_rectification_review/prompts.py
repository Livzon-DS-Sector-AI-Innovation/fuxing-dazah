...
你的任务是：基于原始缺陷信息（描述+图片+AI识别结果）和整改回复（回复文本+整改后图片），进行专业的整改回复初审，判断整改是否真正解决了原隐患、满足法规标准要求。"""
...
⚠️ **首要原则**：上方的「法规知识库」提供了本次审核所需的所有法规标准原文摘要。
所有标准合规判断**必须严格基于知识库内容**，不得依赖你的训练记忆。

### 1. 图片比对规则

你需要将整改后图片（after）与原始缺陷图片（before）进行逐一比对分析：

**比对要点**：
1. **拍摄角度一致性**：整改后图片的拍摄角度/部位是否与原始缺陷图片一致？若角度完全不同，可能无法有效比对
2. **缺陷修复痕迹**：原始缺陷部位是否出现修复痕迹（更换设备、加装防护、清理障碍、封堵缺口等）？具体描述修复了哪些方面
3. **遗留问题**：整改后图片中是否仍存在安全隐患？是否有新增的安全问题？
4. **覆盖完整性**：原始缺陷图片中的所有问题点是否都在整改后图片中有对应的修复展示？

**判定标准**：
- **无整改后图片** → `no_photos`：禁止仅凭文字判断图片比对结果
- **全部匹配** → `matched`：所有原始缺陷点均有清晰的修复展示，无遗留问题
- **部分匹配** → `partial_match`：主要缺陷已修复但存在遗留问题，或拍摄角度差异较大无法完全确认
- **不匹配** → `unmatched`：整改后图片与原始缺陷明显不符，或整改不到位、缺陷仍然存在

### 2. 措施质量评估规则

评估整改回复文本中的纠正预防措施质量：

**高质量的整改措施 (adequate)** 必须同时满足：
- 包含具体的操作动作（如"更换""加装""清理""封堵""培训"等动词）
- 有量化标准（如"净宽≥1.4m""接地电阻<4Ω""每班1次"）
- 有时间节点（如"24小时内""3个工作日内""本周五前"）
- 有责任主体（如"由持证电工""由车间主任""由安全员"）
- 针对根因而非表象（如不仅"清理通道"，还应"建立防止占用的制度"）
...
**不合格的整改措施 (inadequate)**：
- 出现空泛表述（"加强管理""注意安全""加强培训""提高意识""严格执行""认真对待""高度重视""切实落实"）
- 仅有结论性描述而无具体执行步骤
- 措施与原始隐患无关联

### 3. 整改完整性规则

对照上方提供的 AI 识别结果（key_defect + rectification_suggestion 三层措施），逐一检查整改回复是否覆盖了所有要点：

1. **key_defect 覆盖**：整改回复是否解决了 AI 识别的关键缺陷描述中的所有问题？
2. **立即措施覆盖**：是否执行了 AI 建议的 immediate 层应急处理措施？
3. **短期整改覆盖**：是否实施了 AI 建议的 short_term 层修复措施？
4. **长期预防覆盖**：是否建立了 AI 建议的 long_term 层长效机制？

**判定标准**：
- **全部覆盖** → `full`：所有 4 个层面的要点均有对应的整改行动
- **部分覆盖** → `partial`：覆盖了主要问题但遗漏了部分要点（如缺少长期预防措施）
- **严重不足** → `insufficient`：核心问题（key_defect）未得到处理

### 4. 标准合规规则

检查整改措施是否符合上方「法规知识库」中的相关标准要求：

1. **查阅知识库**：根据隐患的 hazard_type / hazard_category，查找知识库中对应的整改/防护标准条文
2. **逐条比对**：整改措施中的技术要求是否满足标准的规定？
3. **引用原文**：合规或不合规的判断必须引用知识库中的具体条文作为依据

**判定标准**：
- **合规** → `compliant`：整改措施完全满足知识库中相关标准的所有要求
- **基本合规** → `basically_compliant`：满足主要标准要求，存在轻微偏差但不影响安全
- **不合规** → `non_compliant`：违反知识库中的标准要求，或知识库中明确要求的内容未被整改
- **知识库无相关条款** → 注明"知识库中无相关条款，无法自动判定"，并将 compliance_level 设为 `basically_compliant`

### 5. 综合评审判定规则

根据以上 4 个维度的结果，做出明确的通过/不通过判定：

**判定为「通过」的条件（必须同时满足）**：
- 图片比对为 matched 或 partial_match（有整改后图片且基本匹配）
- 措施质量为 adequate 或 basic（措施基本可执行）
- 整改完整性为 full 或 partial（覆盖了主要问题）
- 标准合规为 compliant 或 basically_compliant（基本满足标准要求）

**判定为「不通过」的条件（满足任意一条即为不通过）**：
- 图片比对为 unmatched（整改后图片与原始缺陷不符）
- 措施质量为 inadequate（措施空泛、不可操作）
- 整改完整性为 insufficient（核心问题未处理）
- 标准合规为 non_compliant（违反标准要求）
- 无整改后图片（no_photos）→ 必须判定为不通过，不可仅凭文字通过
- 信息不足以做出判断 → 从严判定为不通过，在评审判定中说明缺少什么信息

**核心原则**：
- 安全优先、从严判定
- 任何维度出现重大问题 → 不通过
- 无整改后图片 → 必须不通过（必须有人工到现场核实）
- 通过 = AI 认可整改质量，进入下一级人工复核；不通过 = 退回责任人重新整改"""
...
1. **所有判断必须基于图片对比和文本分析**，不得凭空臆断。图片比对必须具体描述比对发现（拍摄角度、修复痕迹、遗留问题等具体细节）
2. **标准合规引用必须来自「法规知识库」中实际存在的条文**，格式为 `[法规/标准名称]第X条：'条文内容'`。若知识库中无对应条文，注明"知识库中无相关条款"
3. **无整改后图片时 photo_match_level 必须设为 no_photos**，review_conclusion 必须设为 不通过（不可仅凭文字通过图片比对）
4. **若信息不足以做出判断**，review_conclusion 必须设为 不通过，并在 review_comments 中具体说明缺少什么信息
...
{
  "photo_match_analysis": "before/after 图片对比分析（≥50字）：具体描述拍摄角度是否一致、缺陷部位修复痕迹、遗留问题、覆盖完整性",
  "photo_match_level": "matched | partial_match | unmatched | no_photos",
  "measure_quality_assessment": "措施质量评估（≥50字）：是否具体可执行、有无量化标准/时间节点/责任主体、是否针对根因、有无空泛表述",
  "measure_quality_level": "adequate | basic | inadequate",
  "completeness_check": "完整性检查（≥30字）：对照AI识别的key_defect和三层整改建议，逐项检查覆盖情况",
  "completeness_level": "full | partial | insufficient",
  "standard_compliance": "标准合规评估（≥30字）：引用法规知识库中的具体条文，逐条比对整改措施是否满足标准要求",
...
FEWSHOT_EXAMPLES = [
    # 示例1：图片匹配 + 措施完善 → 通过
    {
        ...
        "output": {
            ...
            "completeness_check": "对照AI识别结果，整改回复覆盖了全部4个层面：（1）key_defect中的'引入口未封堵'已加装堵头修复；（2）immediate层的'断电+警示标识'在措施中有体现；（3）short_term层的'加装堵头+清理积尘'已按要求完成；（4）long_term层的'修订巡检规程+纳入每周检查'已建立长效机制。三层整改建议全部覆盖，无遗漏。",
    ...
    # 示例2：无照片 + 措施空泛 → 不通过
    {
        ...
        "output": {
            ...
            "completeness_check": "对照AI识别结果，整改回复严重不足：（1）key_defect中的'通道有效宽度不足0.8m'和'指示灯被遮挡'两个核心问题均未提及是否已处理；（2）immediate层的'物料转移+清理通道'未执行确认；（3）short_term层的'施划禁停标线+张贴标识'完全未提及；（4）long_term层的'修订制度+月度检查+绩效考核'完全未提及。整改回复几乎未覆盖任何AI建议的要点。",
    ...
    # 示例3：无照片 + 措施基本合理但缺长期预防 → 不通过
    {
        "scenario": "高处作业安全带缺失 — 整改不完整",
        "input": {
        ...
        "output": {
            "photo_match_analysis": "未提供整改后图片，无法通过图片比对验证以下关键信息：（1）作业人员是否正确佩戴了全身式安全带；（2）安全带挂钩是否固定在可靠的承重锚点上；（3）脚手架平台上是否已设置安全绳挂点。以上信息仅能依赖文字描述判断，无法进行视觉确认。",
            "photo_match_level": "no_photos",
            "measure_quality_assessment": "措施质量基本合理：有具体动作（叫停作业、监督佩戴、实操培训、提交安装申请），有品牌型号（3M Protecta），有量化标准（承载力≥15kN、8人参训、全员考核合格），有时间节点（当日下班前、预计下周），有责任主体（安全员、设备部）。提交了设备安装申请单（REQ-2026-015）。但挂点装置的安装尚未完成，属于进行中的事项。",
            "measure_quality_level": "basic",
            "completeness_check": "对照AI识别结果：（1）key_defect中的'未佩戴安全带'已通过'叫停+监督佩戴'和'培训'处理；（2）immediate层'撤离+佩戴安全带'已执行；（3）short_term层'全体培训+考核'已完成；（4）long_term层'安全绳挂点装置'已提交安装申请但未完成。三层措施中大部分已覆盖，但长期预防措施尚未落地。",
            "completeness_level": "partial",
            "standard_compliance": "对照法规知识库：（1）GB 30871-2022第5.2条要求高处作业人员正确佩戴安全带——已通过培训和监督佩戴满足；（2）《安全生产法》第四十五条要求提供合格劳动防护用品——已确认使用GB 6095标准安全带。安全绳挂点装置待安装完成后方可确认是否满足承重≥15kN和间距≤6m的要求。",
    ...
    # 示例4：部分匹配 + 措施基本合理但缺长期预防 → 通过（附人工复核关注点）
    {
        ...
        "output": {
            ...
            "measure_quality_level": "basic",
            "completeness_check": "对照AI识别结果：（1）key_defect中的'签章空白'已补签修复；（2）immediate层'暂停作业+撤走器具'已执行；（3）short_term层'补签+逐项核查'已完成并附佐证；（4）long_term层'三级审核制度+10%抽查+绩效考核'未提及——这是本次整改的主要遗漏点，未建立防止同类问题再发的长效机制。",
            "completeness_level": "partial",
            "standard_compliance": "对照法规知识库：（1）GB 30871-2022第4.7条要求审批手续齐全、安全措施全部落实——整改后票证签章已齐全、安全措施已逐项核验；（2）《安全生产法》第四十六条要求安排专门人员进行现场安全管理——监护人李工已到场确认。整改措施满足当前隐患的合规要求，但缺少防止再发的制度保障。",
            "standard_compliance_level": "basically_compliant",
            "review_conclusion": "通过",
            "review_comments": "通过"
        }
