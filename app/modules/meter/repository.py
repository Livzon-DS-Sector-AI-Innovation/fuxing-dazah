...
async def list_instruments(
    ...
    *,
    ...
    if department:
        query = query.where(InstrumentRecord.department.ilike(f"%{department}"))
    if asset_number:
        query = query.where(InstrumentRecord.asset_number == asset_number)
    if instrument_name:
        query = query.where(InstrumentRecord.instrument_name.ilike(f"%{instrument_name}%"))
    if model_spec:
        query = query.where(InstrumentRecord.model_spec == model_spec)
    if measurement_range:
        query = query.where(InstrumentRecord.measurement_range == measurement_range)
    if accuracy_grade:
        query = query.where(InstrumentRecord.accuracy_grade == accuracy_grade)
    if serial_number:
        query = query.where(InstrumentRecord.serial_number == serial_number)
    if location:
        query = query.where(InstrumentRecord.location == location)
    if manufacturer:
        query = query.where(InstrumentRecord.manufacturer == manufacturer)
    if status:
        if status == "超期":
            query = query.where(
                or_(
                    InstrumentRecord.status == "超期",
                    (InstrumentRecord.status == "在用") & (InstrumentRecord.next_calibration_date < date.today()),
                )
            )
        else:
            query = query.where(InstrumentRecord.status == status)
    if calibration_unit:
        query = query.where(InstrumentRecord.calibration_unit == calibration_unit)
    if calibration_result:
        query = query.where(InstrumentRecord.calibration_result == calibration_result)
    if color_marking:
        query = query.where(InstrumentRecord.color_marking == color_marking)
    ...
    query = query.order_by(InstrumentRecord.updated_at.desc(), InstrumentRecord.id.desc())
...
async def list_gas_detectors(
    ...
    *,
    ...
    if department:
        query = query.where(GasDetectorRecord.department.ilike(f"%{department}"))
    if instrument_name:
        query = query.where(GasDetectorRecord.instrument_name.ilike(f"%{instrument_name}%"))
    if detection_model:
        query = query.where(GasDetectorRecord.detection_model == detection_model)
    if product_number:
        query = query.where(GasDetectorRecord.product_number == product_number)
    if installation_type:
        query = query.where(GasDetectorRecord.installation_type == installation_type)
    if installation_location:
        query = query.where(GasDetectorRecord.installation_location == installation_location)
    if medium:
        query = query.where(GasDetectorRecord.medium == medium)
    if detection_unit:
        query = query.where(GasDetectorRecord.detection_unit == detection_unit)
    if calibration_result:
        query = query.where(GasDetectorRecord.calibration_result == calibration_result)
    if calibration_factor:
        query = query.where(GasDetectorRecord.calibration_factor == calibration_factor)
    if manufacturer_supplier:
        query = query.where(GasDetectorRecord.manufacturer_supplier == manufacturer_supplier)
    if manufacturer:
        query = query.where(GasDetectorRecord.manufacturer == manufacturer)
    if next_calibration_before:
    ...
    query = query.order_by(GasDetectorRecord.updated_at.desc(), GasDetectorRecord.id.desc())
    query = query.offset((page - 1) * page_size).limit(page_size)
    result = await db.execute(query)
    return list(result.scalars().all()), total
...
async def get_gas_detector_filter_options(
    ...
    columns = [
        ...
        "detection_unit", "calibration_result",
    ]
    result: dict[str, list[str]] = {}
...
async def get_instrument_overview(db: AsyncSession) -> dict[str, int]:
    ...
    # 子查询计算每条记录的有效状态
    status_expr = case(
        (InstrumentRecord.status == "超期", "超期"),
        (
            (InstrumentRecord.status == "在用") & (InstrumentRecord.next_calibration_date < today),
            "超期",
        ),
...
async def get_gas_detector_overview(db: AsyncSession) -> dict[str, int]:
    ...
    base = select(
        func.count().label("total"),
        func.count().label("in_use"),  # 探测器无 status 字段，全部计为在用
        func.sum(
            case(
                (
                    GasDetectorRecord.next_calibration_date.isnot(None)
        ...
        func.sum(0).label("stopped"),  # 探测器无停用状态
        func.sum(
            case(
                (
                    GasDetectorRecord.next_calibration_date.isnot(None)
    ...
    return {
        "total": total,
        "in_use": total - overdue,  # 探测器：在用 = 总数 - 超期
        "overdue": overdue,
        "stopped": 0,
        "due_today": row.due_today or 0,
        "due_7d": row.due_7d or 0,
