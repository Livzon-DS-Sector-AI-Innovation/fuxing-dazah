...
import { GasDetectorRecord, GasDetectorFilter } from '@/types/meter'
import { deleteGasDetector, getGasDetectors, exportGasDetectorReports, exportGasDetectorsExcel, searchGasDetectorTypeahead, batchDeleteGasDetectors, getGasDetectorIds } from '@/actions/meter'
...
const GAS_DETECTOR_FILTER_KEY: Record<string, keyof GasDetectorFilter> = {
  ...
  detection_model: 'detection_model_like',
  product_number: 'product_number_like',
  measurement_range: 'measurement_range_like',
  installation_type: 'installation_type_like',
  installation_location: 'installation_location_like',
  medium: 'medium_like',
  calibration_factor: 'calibration_factor_like',
  manufacturer_supplier: 'manufacturer_supplier_like',
  manufacturer: 'manufacturer_like',
  status: 'status',
  detection_unit: 'detection_unit_like',
...
const fetchGasDetectorDeptOptions = async () =>
  (await searchGasDetectorTypeahead('department', '', 200)).items
const fetchGasDetectorStatusOptions = async () =>
  (await searchGasDetectorTypeahead('status', '', 200)).items
...
export function GasDetectorTable() {
  ...
  const textFilter = (field: string, label: string) =>
    renderTextFilterDropdown({
      ...
      fetchTypeahead: (q) => searchGasDetectorTypeahead(field, q),
    })
  ...
  const columns: TableColumnsType<GasDetectorRecord> = [
    {
      ...
      filterDropdown: multiFilter('department', '部门', fetchGasDetectorDeptOptions),
      onFilter: () => true,
    },
    {
    ...
    },
    {
      ...
      filterDropdown: textFilter('detection_model', '检测型号'),
      onFilter: () => true,
    },
    { title: '量程', dataIndex: 'measurement_range', width: 120, ellipsis: true,
      ...
      filterDropdown: textFilter('measurement_range', '量程'),
      onFilter: () => true,
    },
    {
      ...
      filterDropdown: textFilter('product_number', '产品编号'),
      onFilter: () => true,
    },
    {
      ...
      filterDropdown: textFilter('installation_type', '安装方式'),
      onFilter: () => true,
    ...
    },
    {
      ...
      filterDropdown: textFilter('calibration_factor', '标定系数'),
      onFilter: () => true,
    },
    {
      ...
      filterDropdown: textFilter('manufacturer_supplier', '传感器出厂日期'),
      onFilter: () => true,
    },
    {
      ...
      filterDropdown: textFilter('manufacturer', '制造单位'),
      onFilter: () => true,
    },
    {
      title: '状态', dataIndex: 'status', width: 80,
      filteredValue: columnFilters.status ? columnFilters.status.split(',') : null,
      filterDropdown: multiFilter('status', '状态', fetchGasDetectorStatusOptions),
      onFilter: () => true,
    ...
    },
    {
      ...
      filterDropdown: textFilter('detection_unit', '检测单位'),
      onFilter: () => true,
    },
    {
