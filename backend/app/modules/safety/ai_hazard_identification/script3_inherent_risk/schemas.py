...
class LECOutput(BaseModel):
    ...
    风险等级判定（统一 Bitable 阈值）：
    - D ≥ 160  → level_1（重大风险）
    - 70 ≤ D < 160 → level_2（较大风险）
    - 20 ≤ D < 70 → level_3（一般风险）
    - D < 20 → level_4（低风险）
    """
...
RISK_LEVEL_TABLE = """
风险等级判定:
  D ≥ 160       → level_1（一级/重大风险）   — 公司级管控
  70 ≤ D < 160  → level_2（二级/较大风险）   — 部门级管控
  20 ≤ D < 70   → level_3（三级/一般风险）   — 班组/岗位级管控
         D < 20 → level_4（四级/低风险）      — 班组/岗位级管控
"""
