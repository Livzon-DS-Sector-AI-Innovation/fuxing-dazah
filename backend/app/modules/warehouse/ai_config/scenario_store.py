"""AI 场景配置 store — 场景级开关与模型绑定运行时大脑（镜像 safety/ai_config/scenario_store.py）。

- **读（热路径，全同步）**：``get_scenario_view`` 只读内存缓存；缓存 miss / TTL
  过期时经同步只读会话重建（每场景一次查询）。
- **场景视图解析**：enabled 缺省 true；model_profile 缺省 NULL（空串视为 None）
  → ``effective_profile`` = 绑定 ∈ 白名单时取绑定，否则回退默认（agent）；
...
- **写（async，API 层传入 session）**：``set_scenario`` 校验（enabled bool、
  ...
  （无密钥不脱敏）→ flush → select re-fetch → invalidate(scenario)。
...
from app.modules.warehouse.models import AiScenarioConfig, AiScenarioConfigAudit
...
ScenarioRowLoader = Callable[[str], Any]
...
class AiScenarioStore:
    ...
    def __init__(self, row_loader: ScenarioRowLoader | None = None) -> None:
    ...
    def get_scenario_view(self, scenario: str) -> ScenarioView:
        """某场景合并视图；未注册场景/无 DB 行 → default-enabled（不抛异常）。"""
        now = time.monotonic()
        ...
        view = self._build_scenario_view(
            scenario, scenario_registry.SCENARIO_REGISTRY.get(scenario),
            self._fetch_scenario_row(scenario),
        )
    ...
    def iter_scenario_views(self) -> tuple[ScenarioView, ...]:
        """遍历 registry 全部场景，逐场景合并视图（供 GET ai-scenarios 全量）。"""
        return tuple(
    ...
    async def set_scenario(
        ...
        - 未知场景抛 ValueError（404 语义）；
        - 白名单外绑定抛 ValueError（422 语义）；
        - enabled 必须 bool；model_profile 可为 None/空串（清绑定）；note ≤255。
        """
        info = scenario_registry.get_scenario_info(scenario)  # 未知场景抛 ValueError
        values = self._validate_scenario_payload(scenario, payload)
        ...
        return self._build_scenario_view(
            scenario, info, fresh if fresh is not None else row
        )
    ...
    def _build_scenario_view(
        ...
        if info is None:
            ...
            label, description, model_type, channel = "", "", "text", "unknown"
            allowed: tuple[str, ...] = ("agent", "agent_backup")
    ...
    def _fetch_scenario_row(self, scenario: str) -> AiScenarioConfig | None:
        """DB 活行查询（同步只读）。失败仅告警并返回 None（读路径回退默认开启）。"""
        try:
            if self._row_loader is not None:
                return cast("AiScenarioConfig | None", self._row_loader(scenario))
    ...
    def _validate_scenario_payload(
        ...
        """校验并归一化 set_scenario 增量（API 层映射 404/422）。"""
        allowed = scenario_registry.allowed_profiles_for(scenario)
        values: dict[str, Any] = {}
    ...
    def _compact_scenario(row: AiScenarioConfig) -> dict[str, Any]:
        """审计 before/after：{enabled, model_profile, note} compact（无密钥不脱敏）。"""
        return {
            "enabled": row.enabled,
