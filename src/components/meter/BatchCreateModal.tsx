...
export function BatchCreateModal({ open, source, onClose }: Props) {
  ...
  const columns: TableColumnsType<RowItem> = useMemo(() => {
    ...
    return [
      ...
      { title: '传感器出厂日期', width: 150, render: (_: unknown, r: RowItem, i: number) => <Input size="small" value={r.manufacturer_supplier ?? ''} onChange={e => updateRow(i, 'manufacturer_supplier', e.target.value || null)} /> },
