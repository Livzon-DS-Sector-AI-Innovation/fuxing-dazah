...
class ZhihengWaterAdapter(BasePlatformAdapter):
    ...
    async def fetch_energy_data(
        ...
        # 4. 逐个水表请求 API：日汇总模式返回逐小时字典，小时模式过滤单个小时
        hour_data: dict[str, dict[int, float]] = {}  # {meter_id: {hour: value}}
        meter_errors: set[str] = set()
        async with httpx.AsyncClient(timeout=30.0) as client:
            for meter_id in sorted(all_meter_ids):
                try:
                    hour_data[meter_id] = await _fetch_meter_data(
        ...
        if is_daily:
            ...
            for hour in range(24):
                target_dt = target_hour.replace(hour=hour)
                meter_values: dict[str, float] = {
                    mid: hour_data.get(mid, {}).get(hour, 0.0)
                    for mid in all_meter_ids
                }
                for code in device_codes:
                    value = (
                        eval_formula(code, meter_values)
                        if has_formula(code)
                        else meter_values.get(code, 0.0)
                    )
                    results.append(
                        CollectResult(
                            device_code=code,
            ...
            # 小时模式：单小时值
            meter_values = {
                mid: hour_data.get(mid, {}).get(target_hour.hour, 0.0)
                for mid in all_meter_ids
            }
            for code in device_codes:
                value = (
...
async def _fetch_meter_data(
    ...
    while True:
        try:
            ...
            logger.exception("智恒 API HTTP 错误: emtrnum=%s, page=%d", emtrnum, page)
            page += 1
            if page > MAX_PAGES:
                logger.warning("水表 %s 分页请求连续失败已达上限 %d 页，停止", emtrnum, MAX_PAGES)
                break
            continue
        except (json.JSONDecodeError, RuntimeError) as e:
            logger.warning("智恒 API 响应解析错误: emtrnum=%s, page=%d, error=%s", emtrnum, page, e)
            page += 1
            if page > MAX_PAGES:
                break
            continue

    mode = "全天按小时" if target_hour is None else f"小时 {target_hour.isoformat()}"
    total_val = sum(hour_values.values())
    logger.debug(
        "水表 %s [%s]: %d 条记录, 匹配 %d 条, 合计=%.4f",
        emtrnum, mode, fetched, matched_count, total_val,
    )

    if total_val == 0.0 and matched_count > 0:
        logger.warning(
            "水表 %s [%s] 采集值为 0（匹配 %d 条记录但 AANALOGFLOW 全部为 0），请确认水表读数是否正常。",
            emtrnum, mode, matched_count,
        )
    elif total_val == 0.0:
        logger.warning(
            "水表 %s [%s] 采集值为 0（%d 条记录中无匹配记录），请检查 API 数据时间是否对齐。",
