"""识别/登记 Pipeline（S2：recognizer → aligner → draft_flow → submit；
S3：SCENE_CONFIG 场景配置 + 对话收集 create_dialog_draft + submit_gmp/
submit_outbound；V3.0 §4.6：finished_receipt 成品入库识别+对话双入口）。

对外 re-export 稳定接口，内部模块经 ``warehouse.agent.pipeline`` 引入。
draft_flow 导入即注册各 scene 确认回调（submit 真身，draft_flow 注册处
薄包装延迟 import 解 cards↔submit 模块环）。
"""

from app.modules.warehouse.agent.pipeline.aligner import (
    FUZZY_MIN_RATIO,
    MASTER_CACHE_TTL,
    AlignedReceipt,
    MaterialMasterEntry,
    align_batch,
    align_receipt,
    get_master_entries,
    match_material,
    normalize_name,
)
from app.modules.warehouse.agent.pipeline.draft_flow import (
    ACTIVE_STATUSES,
    DRAFT_NO_RETRIES,
    FINISHED_OUTBOUND_SCENE,
    FINISHED_RECEIPT_SCENE,
    GMP_OUTBOUND_SCENE,
    PICKING_OUTBOUND_SCENE,
    RECEIPT_SCENE,
    SCENE_CONFIG,
    DraftFlowError,
    SceneConfig,
    cancel_draft,
    create_dialog_draft,
    create_receipt_draft,
    expire_stale,
    mark_aligned,
    mark_direct_aligned,
    send_confirm_card,
)
from app.modules.warehouse.agent.pipeline.recognizer import (
    DOC_TYPE_FINISHED,
    DOC_TYPE_RAW,
    FINISHED_OPTIONAL_FIELDS,
    FINISHED_REQUIRED_FIELDS,
    OPTIONAL_FIELDS,
    RECOGNIZE_MAX_TOKENS,
    RECOGNIZE_PROMPT,
    RECOGNIZE_TEMPERATURE,
    REQUIRED_FIELDS,
    RecognizedField,
    RecognizedFinishedReceipt,
    RecognizedReceipt,
    build_finished_receipt,
    build_receipt,
    build_vision_message,
    classify_document,
    parse_receipt_payload,
    recognize_finished_receipt,
    recognize_receipt,
    required_all_missing,
)
from app.modules.warehouse.agent.pipeline.submit import (
    ARRIVAL_STATUS_VALUE,
    ATTACHMENT_FIELD,
    CHECK_FIELDS,
    FINISHED_CHECK_FIELDS,
    FINISHED_OUTBOUND_TABLE,
    FINISHED_RECEIPT_CHECK_FIELDS,
    FINISHED_RECEIPT_TABLE,
    GMP_CHECK_FIELDS,
    GMP_OUTBOUND_TABLE,
    PICKING_CHECK_FIELDS,
    PICKING_OUTBOUND_TABLE,
    RECEIPT_TABLE,
    build_finished_fields,
    build_finished_receipt_fields,
    build_gmp_fields,
    build_picking_fields,
    build_receipt_fields,
    submit_finished_receipt,
    submit_gmp,
    submit_outbound,
    submit_picking,
    submit_receipt,
)

__all__ = [
    "ACTIVE_STATUSES",
    "ARRIVAL_STATUS_VALUE",
    "ATTACHMENT_FIELD",
    "AlignedReceipt",
    "CHECK_FIELDS",
    "DOC_TYPE_FINISHED",
    "DOC_TYPE_RAW",
    "DRAFT_NO_RETRIES",
    "DraftFlowError",
    "FINISHED_CHECK_FIELDS",
    "FINISHED_OPTIONAL_FIELDS",
    "FINISHED_OUTBOUND_SCENE",
    "FINISHED_OUTBOUND_TABLE",
    "FINISHED_RECEIPT_CHECK_FIELDS",
    "FINISHED_RECEIPT_SCENE",
    "FINISHED_RECEIPT_TABLE",
    "FINISHED_REQUIRED_FIELDS",
    "FUZZY_MIN_RATIO",
    "GMP_CHECK_FIELDS",
    "GMP_OUTBOUND_SCENE",
    "GMP_OUTBOUND_TABLE",
    "MASTER_CACHE_TTL",
    "MaterialMasterEntry",
    "OPTIONAL_FIELDS",
    "PICKING_CHECK_FIELDS",
    "PICKING_OUTBOUND_SCENE",
    "PICKING_OUTBOUND_TABLE",
    "RECEIPT_SCENE",
    "RECEIPT_TABLE",
    "RECOGNIZE_MAX_TOKENS",
    "RECOGNIZE_PROMPT",
    "RECOGNIZE_TEMPERATURE",
    "REQUIRED_FIELDS",
    "SCENE_CONFIG",
    "SceneConfig",
    "RecognizedField",
    "RecognizedFinishedReceipt",
    "RecognizedReceipt",
    "align_batch",
    "align_receipt",
    "build_finished_fields",
    "build_finished_receipt",
    "build_finished_receipt_fields",
    "build_gmp_fields",
    "build_picking_fields",
    "build_receipt",
    "build_receipt_fields",
    "build_vision_message",
    "cancel_draft",
    "classify_document",
    "create_dialog_draft",
    "create_receipt_draft",
    "expire_stale",
    "get_master_entries",
    "mark_aligned",
    "mark_direct_aligned",
    "match_material",
    "normalize_name",
    "parse_receipt_payload",
    "recognize_finished_receipt",
    "recognize_receipt",
    "required_all_missing",
    "send_confirm_card",
    "submit_finished_receipt",
    "submit_gmp",
    "submit_outbound",
    "submit_picking",
    "submit_receipt",
]
