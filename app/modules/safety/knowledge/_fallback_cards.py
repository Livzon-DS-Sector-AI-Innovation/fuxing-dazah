"""Hardcoded fallback knowledge cards — 26 regulatory document summaries.

Extracted from KnowledgeInjector._build_fallback_cards() to keep injector.py
under the ~300-line threshold. These cards are deployed with the code so AI
recognition works immediately on new deployments without requiring DB seeding.

Coverage:
  - AI Hazard Identification Harness Section 5.1: 13 documents
  - Hazard Source Identification required standards: 15 documents
  - Overlap: Safety Production Law, GB/T 13861, Special Equipment Safety Law

Organization:
  ① P0 cards (5) — Core judgment and classification criteria
  ② P1 cards (12) — Category judgment and rectification measures
  ③ P2 cards (8 + 1 methodology) — Specialized technical standards and supplements
"""
...
def build_fallback_cards() -> list[KnowledgeCard]:
    """Return 26 hardcoded fallback cards (P0/P1/P2) covering 25 regulatory documents + LEC methodology.

    These cards ship with the code so AI recognition works immediately on
    new deployments. DB-stored cards are incremental optimizations with more
    complete original citations — they take priority over these fallbacks.

    Coverage: AI hazard identification (6 dimensions) + risk source identification
    (per-script category filtering via build_context).
    """
    return [
        # ═══════════════════════════════════════════════════════════════
        # P0（5 张）—— 核心判定与分类标准
