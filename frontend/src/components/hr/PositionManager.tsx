...
export default function PositionManager() {
  ...
  const uniqueDepts = [...new Set(positions.map(p => p.department))]
  const deptFilterOptions = uniqueDepts.map(d => ({ text: d, value: d }))
  const deptSelectOptions = uniqueDepts.map(d => ({ value: d, label: d }))
    ...
    <div className="space-y-4">
      <Card size="small">
        <div className="flex items-center justify-between mb-3">
          <Space wrap>
            <Select placeholder="按部门筛选" allowClear style={{ width: 180 }}
              value={filterDept} onChange={setFilterDept} options={deptSelectOptions} />
            <Input placeholder="部门" style={{ width: 150 }}
              value={newDept} onChange={e => setNewDept(e.target.value)}
              list="dept-list" />
            <datalist id="dept-list">{deptSelectOptions.map(d => <option key={d.value} value={d.value} />)}</datalist>
        ...
        <Table
          ...
          columns={[
            { title: '部门', dataIndex: 'department', width: 200, filters: deptFilterOptions, onFilter: (v, r: PositionRow) => r.department === v,
